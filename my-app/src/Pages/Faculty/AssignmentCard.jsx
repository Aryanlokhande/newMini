...
function AssignmentCard({isChecked,asg,dead,sendToCol}){
    ...
    return(
        <>
            <div id="details">
                        Assignment Name: <input type="text" id="assignmentname" value={asgnName} onChange={handleNameChange}/>
                        Deadline: <input type="date" value={deadLine} onChange={handleDateChange} /> 
            </div>
            <div className="assignmentdetails"></div>            
            <div className={isChecked===true && (asgnName===asg||asgnName==="") && (deadLine===dead||deadLine==="")?"filter":"filter notFulfill"} ></div> 
        </>
    )
}
